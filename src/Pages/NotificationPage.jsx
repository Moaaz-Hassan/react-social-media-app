...
function NotificationPage() {
  const [selsectAll, setselsectAll] = useState(true);
  ...
  const {
    ...
    queryKey: ["getNotification", selsectAll],
    queryFn: ({ pageParam }) =>
      GetNotifications({ pageParam, unread: !selsectAll }),
    initialPageParam: 1,
  ...
  return (
    <div className=' bg-white rounded-xl border border-blue-500/50 shadow-xl mt-4'>
      <header className='p-4 border-b border-b-gray-500/50'>
        ...
        <div className=' flex gap-2 mt-3'>
          <Button onPress={() => setselsectAll(true)} className="border border-blue-500" color={selsectAll ? `primary` : "white"} size='sm'>All</Button>
          <Button onPress={() => setselsectAll(false)} className="border border-blue-500" color={!selsectAll ? `primary` : "white"} size='sm'>UnRead</Button>
        </div>
